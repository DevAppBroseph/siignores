...
class ForgotPasswordBloc
  ...
  ForgotPasswordBloc(this.resetPassword, this.sendCodeResetPassword,
      this.verifyCodeResetPassword)
      : super(FPInitialState());

  CurrentStage currentStage = CurrentStage.first;
  String email = '';
  ResetDataEntity resetDataEntity = const ResetDataEntity(uid: '', token: '');
  @override
  Stream<ForgotPasswordState> mapEventToState(
      ForgotPasswordEvent event) async* {
    if (event is FPSendCodeEvent) {
      yield FPBlankState();
      var registered = await sendCodeResetPassword(
          SendCodeResetPasswordParams(email: event.email));
    ...
    if (event is FPVerifyCodeEvent) {
      yield FPBlankState();
      var activated = await verifyCodeResetPassword(
          VerifyCodeResetPasswordParams(email: email, code: event.code));
      yield activated.fold((failure) => errorCheck(failure), (data) {
    ...
    if (event is FPSetPasswordEvent) {
      var activated = await resetPassword(ResetPasswordParams(
          resetDataEntity: resetDataEntity, password: event.password));
