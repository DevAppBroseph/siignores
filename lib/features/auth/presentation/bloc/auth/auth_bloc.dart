...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  AuthBloc(this.authSignIn, this.getUserInfo, this.getTokenLocal, this.logout,
      this.deleteAccount)
      : super(InitialState());
  ...
  Stream<AuthState> mapEventToState(AuthEvent event) async* {
    if (event is CheckUserLoggedEvent) {
      ...
      yield token.fold((failure) => errorCheck(failure), (token) {
        if (token != null) {
    ...
    if (event is GetUserInfoEvent) {
      ...
      yield gotUserInfo.fold((failure) => errorCheck(failure), (userEntity) {
        if (userEntity.firstName == 'unauthorized') {
          print('Token error logout');
          logout(NoParams());
          sl<AuthConfig>().token = null;
          sl<AuthConfig>().authenticatedOption =
              AuthenticatedOption.unauthenticated;
          return CheckedState();
        }
    ...
    if (event is SignInEvent) {
      ...
      yield authGetToken.fold((failure) => errorCheck(failure), (String token) {
        sl<AuthConfig>().token = token;
        return RequiredGetUserInfoState();
      });
    }
    ...
    if (event is LogoutEvent) {
      ...
      yield logoutResult.fold((failure) => errorCheck(failure),
          (bool isExited) {
    ...
    if (event is DeleteAccountEvent) {
      ...
      yield deleteResult.fold((failure) => errorCheck(failure),
          (bool isExited) {
        sl<AuthConfig>().userEntity = null;
        sl<AuthConfig>().token = null;
        sl<AuthConfig>().authenticatedOption =
  ...
  AuthState errorCheck(Failure failure) {
    ...
    if (failure == ConnectionFailure() || failure == NetworkFailure()) {
      ...
      return const ErrorState(message: 'Повторите попытку');
    }
  }
}
