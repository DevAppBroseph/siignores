...
class QuestionTest {
  ...
  QuestionTest({required this.id, required this.title, required this.options, required this.unanswered});
  ...
  factory QuestionTest.fromJson(Map<String, dynamic> json) => QuestionTest(
        ...
        ? []
        : (json['option_set'] as List).map((json) => OptionTest.fromJson(json)).toList()
      );
}
class OptionTest {
  final int id;
